...
import "./Teacherscreen.css"; // use TeacherScreen styles so HTML matches exactly
...
const TeacherLandingPage = () => {
  ...
  const [options, setOptions] = useState(defaultEmptyOptions());
  const [timer, setTimer] = useState("60");
  const [error, setError] = useState("");
  const [currentPoll, setCurrentPoll] = useState(null); // for active poll view
  const [results, setResults] = useState(null);
  const navigate = useNavigate();
  const username = sessionStorage.getItem("username") || "";

  // Socket listeners: poll:new, poll:update_results, poll:error
  useEffect(() => {
    const onNew = (poll) => {
      setCurrentPoll(poll);
      setResults(null);
    ...
    const onResults = (data) => {
      setResults(data.pollResults || data);
    };
    const onError = (err) => {
      alert(err);
    };
    ...
    socket.on("poll:update_results", onResults);
    socket.on("poll:error", onError);

    return () => {
      socket.off("poll:new", onNew);
      socket.off("poll:update_results", onResults);
      socket.off("poll:error", onError);
    };
  }, []);

  // Helpers to update options
  ...
  const toggleCorrect = (index, val) => {
    setOptions((prev) => {
      const next = [...prev];
      next[index] = { ...next[index], correct: val };
      return next;
    });
  };

  const addOption = () => {
    setOptions((prev) => [...prev, { id: prev.length + 1, text: "", correct: null }]);
  };
  ...
  const validateForm = () => {
    ...
    const filledOptions = options.filter((o) => o.text && o.text.trim() !== "");
    if (filledOptions.length < 2) {
      setError("Enter at least 2 options");
      return false;
    }

    // optional: require at least one correct option
  ...
  // Emit to backend — ensure we send objects for options (text + correct)
  const createPoll = () => {
    ...
    // Emit the event your backend appeared to be listening for: "createPoll"
    socket.emit("createPoll", payload);

 
    setQuestion("");
    setOptions(defaultEmptyOptions());
    // Navigate to teacher poll page (keep same behaviour you used previously)
    navigate("/teacher-poll");
  };

  // Render — matches TeacherScreen HTML structure and class names
  return (
    <>
      {!currentPoll ? (
        <>
          <div className="super-container">
            ...
            <div className="que-dur-selector">
              ...
              <div className="selector">
                <select value={timer} name="duration" id="dur-selector" onChange={(e) => setTimer(e.target.value)}>
            ...
            <button className="add-option-btn" onClick={addOption}>+ Add more options</button>

            {error && <div className="alert alert-danger mt-2">{error}</div>}
          </div>

          <div className="footer">
            <button className="ask-question" onClick={createPoll}>
              Ask Question
            </button>
          </div>
        </>
          ...
          <div className="question-container">
            ...
            <div className="inner-container">
              ...
              {currentPoll.options.map((opt, idx) => {
                ...
                const percent = results && total > 0 ? ((results[opt] || 0) / total) * 100 : 0;
                return (
                  <div key={idx}>
                    <div className="vote-bar">
                      <div
                        ...
                        style={{
                          width: `${percent}%`,
                          color: percent > 0 ? "white" : "black",
                        }}
                      >
                        <span className="opt-marker">
                          <span id="opt-num">{idx + 1}</span>
                          <span className="option-text">{opt}</span>
                        </span>
                      </div>
                      <span>{results ? `${Math.round(percent)}%` : "0%"}</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="btn-div">
              <button className="ask-another-btn" onClick={() => setCurrentPoll(null)}>
